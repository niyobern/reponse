    ...
    return (
        <div className="flex flex-col md:grid grid-cols-6 p-2 gap-4 h-min">
            <div className="col-start-1 col-span-4 h-fit flex flex-col" id="mainItem">
                <Image src={extensions[current].images[0]} alt="image" width={900} height={500} className="w-full h-fit object-scale-down"/>
                ...
                {extensions.map((ext: any, i: number) => (
                    <Link href={`/extensions/1${i}`} passHref={true}>
                        <div className="grid grid-rows-6 grid-cols-2 relative text-white bg-green-900 hover:bg-green-600 active:bg-red-900 hover:shadow-2xl hover:shadow-sky-600">
                            <Image src={ext.images[0]} alt="image" width={900} height={500} className="w-full col-span-full row-span-6 z-10"/>
